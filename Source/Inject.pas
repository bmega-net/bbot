...
procedure InjectDll(PID: BInt32; DLL: BPChar);
const
  PROC_ALL_ACCESS = PROCESS_TERMINATE or PROCESS_CREATE_THREAD or PROCESS_VM_OPERATION or PROCESS_VM_READ or
...
begin
  ...
  Param := VirtualAllocEx(hProcess, nil, Length(DLL) + 1, MEM_COMMIT or MEM_RESERVE, PAGE_READWRITE);
  if Param = nil then
  ...
  hThread := CreateRemoteThread(hProcess, nil, 0, pThreadStartRoutine, Param, 0, TID);
  if hThread = 0 then
