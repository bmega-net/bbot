...
type
  ...
  private
    ...
    function ParseFloorDescription(X, Y, Z, Width, Height, Offset: BInt32; var m_skipTiles: BInt32): boolean;
...
begin
  ...
  end else if AdrSelected >= TibiaVer900 then
...
begin
  for Skill := SkillFist to SkillFishing do begin
    Level := Buffer.GetBInt16;
    if AdrSelected >= TibiaVer900 then
  ...
  if AdrSelected > TibiaVer1094 then
    for SpecialSkill := SpecialSkillFirst to SpecialSkillLast do begin
...
begin
  ...
  while Items > 0 do begin
    ID := Buffer.GetBUInt16;
    if (AdrSelected >= TibiaVer872) and (AdrSelected < TibiaVer900) then begin
      ItemAmount := BInt8(Buffer.GetBInt8);
      Count := Buffer.GetBInt16;
    end else begin
      ItemAmount := 0;
...
begin
  ParseMapInfo(Tibia.MapOldPos.X - 8, Tibia.MapNewPos.Y - 6, Tibia.MapNewPos.Z, 18, 1);
end;

procedure TTibiaPackerParser.ParseMapDescriptionSouth;
begin
  ParseMapInfo(Tibia.MapOldPos.X - 8, Tibia.MapNewPos.Y + 7, Tibia.MapNewPos.Z, 18, 1);
end;

procedure TTibiaPackerParser.ParseMapDescriptionWest;
begin
  ParseMapInfo(Tibia.MapNewPos.X - 8, Tibia.MapNewPos.Y - 6, Tibia.MapNewPos.Z, 1, 14);
end;

procedure TTibiaPackerParser.ParseMapDescriptionEast;
begin
  ParseMapInfo(Tibia.MapNewPos.X + 9, Tibia.MapNewPos.Y - 6, Tibia.MapNewPos.Z, 1, 14);
end;
...
var
  Skip: BInt32;
begin
  if Tibia.MapNewPos.Z = 7 then begin
    Skip := -1;
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.X - 6, 5, 18, 14, 3, Skip);
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.X - 6, 4, 18, 14, 4, Skip);
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.X - 6, 3, 18, 14, 5, Skip);
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.X - 6, 2, 18, 14, 6, Skip);
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.X - 6, 1, 18, 14, 7, Skip);
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.X - 6, 0, 18, 14, 8, Skip);
    if Skip >= 0 then begin
      Buffer.GetBInt8;
      Buffer.GetBInt8;
    end;
  end else if Tibia.MapNewPos.Z > 7 then begin
    Skip := -1;
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.Y - 6, Tibia.MapOldPos.Z - 3, 18, 14, 3, Skip);
    if Skip >= 0 then begin
      Buffer.GetBInt8;
      Buffer.GetBInt8;
    end;
...
var
  Skip: BInt32;
begin
  if Tibia.MapNewPos.Z = 8 then begin
    Skip := -1;
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.Y - 6, Tibia.MapNewPos.Z, 18, 14, -1, Skip);
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.Y - 6, Tibia.MapNewPos.Z + 1, 18, 14, -2, Skip);
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.Y - 6, Tibia.MapNewPos.Z + 2, 18, 14, -3, Skip);
    if Skip >= 0 then begin
      Buffer.GetBInt8;
      Buffer.GetBInt8;
    end;
  end else if (Tibia.MapNewPos.Z > Tibia.MapOldPos.Z) and (Tibia.MapNewPos.Z > 8) and (Tibia.MapNewPos.Z < 14) then
  begin
    Skip := -1;
    ParseFloorDescription(Tibia.MapOldPos.X - 8, Tibia.MapOldPos.Y - 6, Tibia.MapNewPos.Z + 2, 18, 14, -3, Skip);
    if Skip >= 0 then begin
...
begin
  ...
  for I := 1 to StepsCount do begin
...
begin
  ...
  if pPos.Y <> BInt32(SlotPurseMarket) then begin
    VerifyPosition(pPos);
    VerifyItem(iID, 1);
  end;
end;
...
begin
  ...
  MESSAGE_PRIVATE_TO, MESSAGE_GAMEMASTER_PRIVATE_TO: begin
      SayData.Receiver := Buffer.GetBStr16;
    end;
  MESSAGE_CHANNEL_MANAGEMENT, MESSAGE_CHANNEL, MESSAGE_CHANNEL_HIGHLIGHT: begin
...
begin
  ...
  if Nature = REPORT_NATURE_NAME then begin
    Reason := Buffer.GetBInt8;
  ...
  end else if Nature = REPORT_NATURE_BOT then begin
    Reason := Buffer.GetBInt8;
    CharName := Buffer.GetBStr16;
    Comment := Buffer.GetBStr16;
...
begin
  ...
  if (not InRange(iFight, 1, 3)) or (not InRange(iChase, 0, 1)) or (not InRange(iSecure, 0, 1)) then
    ReportUnexpectedBehavior(BFormat('Fight Mode %d %d %d', [iFight, iChase, iSecure]), PacketID);
end;
...
procedure TTibiaPackerParser.Client_PurseMarketOpenTransactionHistory(PacketID: BInt32);
begin
  Buffer.GetBInt8; // Entries per page
end;

procedure TTibiaPackerParser.Client_PurseMarketTransactionHistory(PacketID: BInt32);
begin
...
begin
  if AdrSelected >= TibiaVer900 then begin
    ...
    if BugCategory = BUG_CATEGORY_MAP then begin
      Mssage := BFormat('Position: %s\n%s', [BStr(GetPos), Mssage]);
    end else if BugCategory = BUG_CATEGORY_TYPO then begin
...
begin
  ...
  ReportUnexpectedBehavior(Format('Violation %s %s %d/%d/%d', [sName, sComment, iReasonID, iActionID, iChannelID,
    IfThen(bIPBan, 'IP Ban', '-')]), PacketID);
end;
...
begin
  ...
  sComment := Buffer.GetBStr16;
  ReportUnexpectedBehavior(Format('DebugAsset %s %s %s %s', [sDate, sLine, sDescription, sComment]), PacketID);
end;
...
function TTibiaPackerParser.ParseClient: boolean;
  procedure callClientParser(const AName: BStr; const AProc: BUnaryProc<BInt32>; const APacketID: BInt32);
  begin
    FuncName := AName;
    AProc(APacketID);
  end;
...
begin
  ...
  try
    case PacketID of
    $01: callClientParser('Client_Login', Client_Login, PacketID);
    $0F: callClientParser('Client_EnterWorld', Client_EnterWorld, PacketID);
    $0A: callClientParser('Client_Login', Client_Login, PacketID);
    $14: callClientParser('Client_Logout', Client_Logout, PacketID);
    $1D: callClientParser('Client_Ping', Client_Ping, PacketID);
    $1E: callClientParser('Client_PingBack', Client_PingBack, PacketID);
    $1F: callClientParser('Client_PerformanceMetrics', Client_PerformanceMetrics, PacketID);
    $64: callClientParser('Client_AutoMove', Client_AutoMove, PacketID);
    $65: callClientParser('Client_North', Client_North, PacketID);
    $66: callClientParser('Client_East', Client_East, PacketID);
    $67: callClientParser('Client_South', Client_South, PacketID);
    $68: callClientParser('Client_West', Client_West, PacketID);
    $69: callClientParser('Client_StopAutoMove', Client_StopAutoMove, PacketID);
    $6A: callClientParser('Client_NorthEast', Client_NorthEast, PacketID);
    $6B: callClientParser('Client_SouthEast', Client_SouthEast, PacketID);
    $6C: callClientParser('Client_SouthWest', Client_SouthWest, PacketID);
    $6D: callClientParser('Client_NorthWest', Client_NorthWest, PacketID);
    $6F: callClientParser('Client_TurnNorth', Client_TurnNorth, PacketID);
    $70: callClientParser('Client_TurnEast', Client_TurnEast, PacketID);
    $71: callClientParser('Client_TurnSouth', Client_TurnSouth, PacketID);
    $72: callClientParser('Client_TurnWest', Client_TurnWest, PacketID);
    $77: callClientParser('Client_Equip', Client_Equip, PacketID);
    $78: callClientParser('Client_Throw', Client_Throw, PacketID);
    $79: callClientParser('Client_ShopLook', Client_ShopLook, PacketID);
    $7A: callClientParser('Client_ShopBuy', Client_ShopBuy, PacketID);
    $7B: callClientParser('Client_ShopSold', Client_ShopSold, PacketID);
    $7C: callClientParser('Client_ShopClose', Client_ShopClose, PacketID);
    $7D: callClientParser('Client_TradeRequest', Client_TradeRequest, PacketID);
    $7E: callClientParser('Client_TradeLook', Client_TradeLook, PacketID);
    $7F: callClientParser('Client_TradeAccept', Client_TradeAccept, PacketID);
    $80: callClientParser('Client_TradeCancel', Client_TradeCancel, PacketID);
    $82: callClientParser('Client_UseItem', Client_UseItem, PacketID);
    $83: callClientParser('Client_UseItemEx', Client_UseItemEx, PacketID);
    $84: callClientParser('Client_ShootOnBattle', Client_ShootOnBattle, PacketID);
    $85: callClientParser('Client_RotateItem', Client_RotateItem, PacketID);
    $87: callClientParser('Client_ContainerClose', Client_ContainerClose, PacketID);
    $88: callClientParser('Client_ContainerParent', Client_ContainerParent, PacketID);
    $89: callClientParser('Client_WindowText', Client_WindowText, PacketID);
    $8A: callClientParser('Client_WindowHouseText', Client_WindowHouseText, PacketID);
    $8B: callClientParser('Client_WrapToggle', Client_WrapToggle, PacketID);
    $8C: callClientParser('Client_LookAt', Client_LookAt, PacketID);
    $8D: callClientParser('Client_LookAtCreature', Client_LookAtCreature, PacketID);
    $8E: callClientParser('Client_JoinAgression', Client_JoinAgression, PacketID);
    $96: callClientParser('Client_Say', Client_Say, PacketID);
    $97: callClientParser('Client_ChannelList', Client_ChannelList, PacketID);
    $98: callClientParser('Client_ChannelOpen', Client_ChannelOpen, PacketID);
    $99: callClientParser('Client_ChannelClose', Client_ChannelClose, PacketID);
    $9A: callClientParser('Client_ChannelPrivate', Client_ChannelPrivate, PacketID);
    $9B: callClientParser('Client_RuleViolation', Client_RuleViolation, PacketID);
    $9C: callClientParser('Client_EditGuildMessage', Client_EditGuildMessage, PacketID);
    $9D: callClientParser('Client_RuleViolationCancel', Client_RuleViolationCancel, PacketID);
    $9E: callClientParser('Client_NPCClose', Client_NPCClose, PacketID);
    $A0: callClientParser('Client_AttackMode', Client_AttackMode, PacketID);
    $A1: callClientParser('Client_Attack', Client_Attack, PacketID);
    $A2: callClientParser('Client_Follow', Client_Follow, PacketID);
    $A3: callClientParser('Client_PartyInvite', Client_PartyInvite, PacketID);
    $A4: callClientParser('Client_PartyJoin', Client_PartyJoin, PacketID);
    $A5: callClientParser('Client_PartyRevoke', Client_PartyRevoke, PacketID);
    $A6: callClientParser('Client_PartyPassLeader', Client_PartyPassLeader, PacketID);
    $A7: callClientParser('Client_PartyLeave', Client_PartyLeave, PacketID);
    $A8: callClientParser('Client_PartyShared', Client_PartyShared, PacketID);
    $AA: callClientParser('Client_ChannelPrivateCreate', Client_ChannelPrivateCreate, PacketID);
    $AB: callClientParser('Client_ChannelPrivateInvite', Client_ChannelPrivateInvite, PacketID);
    $AC: callClientParser('Client_ChannelPrivateExclude', Client_ChannelPrivateExclude, PacketID);
    $BE: callClientParser('Client_CancelMove', Client_CancelMove, PacketID);
    $C9: callClientParser('Client_RequestUpdateTile', Client_RequestUpdateTile, PacketID);
    $CA: callClientParser('Client_RequestUpdateContainers', Client_RequestUpdateContainers, PacketID);
    $CB: callClientParser('Client_BrowseField', Client_BrowseField, PacketID);
    $CC: callClientParser('Client_SeekContainerPage', Client_SeekContainerPage, PacketID);
    $D2: callClientParser('Client_OutfitRequest', Client_OutfitRequest, PacketID);
    $D3: callClientParser('Client_OutfitSet', Client_OutfitSet, PacketID);
    $D4: callClientParser('Client_MountDismount', Client_MountDismount, PacketID);
    $D5: callClientParser('Client_Embuiment', Client_Embuiment, PacketID);
    $D6: callClientParser('Client_ClearEmbuiment', Client_ClearEmbuiment, PacketID);
    $DC: callClientParser('Client_VipAdd', Client_VipAdd, PacketID);
    $DE: callClientParser('Client_VipEdit', Client_VipEdit, PacketID);
    $DD: callClientParser('Client_VipRemove', Client_VipRemove, PacketID);
    $E6: callClientParser('Client_BugReport', Client_BugReport, PacketID);
    $E7: callClientParser('Client_ViolationWindow', Client_ViolationWindow, PacketID);
    $E8: callClientParser('Client_DebugAsset', Client_DebugAsset, PacketID);
    $E9: callClientParser('Client_StoreEvent', Client_StoreEvent, PacketID);
    $EB: callClientParser('Client_PreyAction', Client_PreyAction, PacketID);
    $EF: callClientParser('Client_TransferCoin', Client_TransferCoin, PacketID);
    $ED: callClientParser('Client_RequestResourceBalance', Client_RequestResourceBalance, PacketID);
    $F0: callClientParser('Client_Quests', Client_Quests, PacketID);
    $F1: callClientParser('Client_QuestsView', Client_QuestsView, PacketID);
    $F2: callClientParser('Client_RuleViolationReport', Client_RuleViolationReport, PacketID);
    $F4: callClientParser('Client_PurseMarketLeave', Client_PurseMarketLeave, PacketID);
    $F5: callClientParser('Client_PurseMarketBrowse', Client_PurseMarketBrowse, PacketID);
    $F6: callClientParser('Client_PurseMarketCreate', Client_PurseMarketCreate, PacketID);
    $F7: callClientParser('Client_PurseMarketCancel', Client_PurseMarketCancel, PacketID);
    $F8: callClientParser('Client_PurseMarketAccept', Client_PurseMarketAccept, PacketID);
    $F9: callClientParser('Client_AnswerModal', Client_AnswerModal, PacketID);
    $FA: callClientParser('Client_PurseMarketOpen', Client_PurseMarketOpen, PacketID);
    $FB: callClientParser('Client_PurseMarketOpenCategory', Client_PurseMarketOpenCategory, PacketID);
    $FC: callClientParser('Client_PurseMarketBuy', Client_PurseMarketBuy, PacketID);
    $FD: callClientParser('Client_PurseMarketOpenTransactionHistory', Client_PurseMarketOpenTransactionHistory,
        PacketID);
    $FE: callClientParser('Client_PurseMarketTransactionHistory', Client_PurseMarketTransactionHistory, PacketID);
  else begin
    ...
    on E: Exception do
      raise BException.Create(String(BFormat('Failed to parse Client 0x%x: %s\n%s', [PacketID, E.Message,
        BufferToStr])));
  end;
...
function TTibiaPackerParser.ParseFloorDescription(X, Y, Z, Width, Height, Offset: BInt32;
...
begin
  ...
  for nX := 0 to (Width - 1) do
    for nY := 0 to (Height - 1) do begin
      if m_skipTiles = 0 then begin
        tileOpt := Buffer.GetBInt16;
        Buffer.Position := Buffer.Position - 2;
        if tileOpt >= $FF00 then begin
...
begin
  ...
  zI := zStart;
  while zI <> (zEnd + zStep) do begin
    if not ParseFloorDescription(X, Y, zI, Width, Height, Z - zI, m_skipTiles) then
      Break;
    zI := (zI + zStep);
  end;
end;
...
begin
  ...
  try
    ...
    $28: ParseDead;
    $5B: ParseSnapBack;
    $64: ParseMapDescription;
    $65: ParseMapDescriptionNorth;
    $66: ParseMapDescriptionEast;
    $67: ParseMapDescriptionSouth;
    $68: ParseMapDescriptionWest;
    $69: ParseMapUpdateTile;
    $6A: ParseTileAdd;
    $6B: ParseTileUpdate;
    $6C: ParseTileRemove;
    $6D: ParseCreatureMove;
    $6E: ParseContainer;
    $6F: ParseContainerClosed;
    $70: ParseContainerAdd;
    $71: ParseContainerUpdate;
    $72: ParseContainerRemove;
    $78: ParseInventorySet;
    $79: ParseInventoryRemove;
    $7A: ParseShop;
    $7B: ParseShopMyItems;
    $7C: ParseShopClosed;
    $7D: ParseTradeItemRequest;
    $7E: ParseTradeItemRequest;
    $7F: ParseTradeClosed;
    $82: ParseWorldLight;
    $83: ParseMagicEffect;
    $84: ParseAnimatedText;
    $85: ParseShoot;
    $86: ParseCreatureSquare;
    $87: ParseTrappers;
    $8C: ParseCreatureHP;
    $8D: ParseCreatureLight;
    $8E: ParseCreatureOutfit;
    $8F: ParseCreatureSpeed;
    $90: ParseCreatureSkull;
    $91: ParseCreatureParty;
    $96: ParseTextWindow;
    $97: ParseHouseWindow;
    $9C: ParseBlessings;
    $9D: ParseSwitchHotkeyPreset;
    $9E: ParsePremiumTrigger;
    $A0: ParseStatsUpdate;
    $A1: ParseSkillsUpdate;
    $A2: ParseStatusFlagsUpdate;
    $A3: ParseCancelTarget;
    $A4: ParseSpellCooldown;
    $A5: ParseGroupCooldown;
    $A6: ParseUseItemDelay;
    $A7: ParseSetTatics;
    $AA: ParseMessageReceived;
    $AB: ParseChannelDialogReceived;
    $AC: ParseChannelOpened;
    $AD: ParsePrivateOpened;
    $AE: ParseEditGuildMessage;
    $AF: ParseRuleViolationsRemove;
    $B0: ParseRuleViolationsCancel;
    $B1: ParseRuleViolationsLock;
    $B2: ParsePrivateChannelCreated;
    $B3: ParsePrivateClosed;
    $B4: ParseSystemMessage;
    $B5: ParseCancelWalk;
    $B6: ParseWalkDelay;
    $B7: ParseUnjustifiedPoints;
    $B8: ParsePVPSituations;
    $BE: ParseMapDescriptionUp;
    $BF: ParseMapDescriptionDown;
    $C8: ParseOutfitWindow;
    $D2: ParseVip;
    $D3: ParseVipLogin;
    $D4: ParseVipLogout;
    $DC: ParseTutorial;
    $DD: ParseAddMark;
    $DF: ParseCreditBalance;
    $E0: ParseMarketShopError;
    $E1: ParseMarketShopInfo;
    $F1: ParseQuestLine;
    $F2: ParseUpdatingShopBalance;
    $F3: ParseChannelEvent;
    $F5: ParsePlayerInventory;
    $F6: ParseMarketEnter;
    $F7: ParseMarketLeave;
    $FB: ParsePremiumShop;
    $FD: ParseTransactionHistory;
    $FE: ParseTransactionSucceed;
    end;
  except
    on E: Exception do
      raise BException.Create(String(BFormat('Failed to parse Server 0x%x: %s\n%s', [PacketID, E.Message,
        BufferToStr])));
  end;
end;
...
procedure TTibiaPackerParser.ReportUnexpectedBehavior(Log: BStr; PacketID: BInt32);
begin
  BBotEngine_SendError(BFormat('Parsing %s %s -> %s [%d..%d]\n%s', [BStr(IntToHex(PacketID, 2)), FuncName, Log,
    Buffer.Position, EOP, BufferToStr]));
end;
...
begin
  ...
  MESSAGE_SAY, MESSAGE_WHISPER, MESSAGE_YELL, MESSAGE_SPELL, MESSAGE_BARK_LOW, MESSAGE_BARK_LOUD,
    MESSAGE_NPC_FROM_START_BLOCK: begin
      MsgData.Position := GetPos;
      MsgData.Text := Buffer.GetBStr16();
    end;
  MESSAGE_PRIVATE_FROM, MESSAGE_GAMEMASTER_PRIVATE_FROM: begin
      MsgData.Channel := 0; // Author;
      MsgData.Text := Buffer.GetBStr16();
    end;
  MESSAGE_CHANNEL, MESSAGE_NPC_FROM, MESSAGE_GAMEMASTER_CHANNEL,
    MESSAGE_CHANNEL_MANAGEMENT, MESSAGE_CHANNEL_HIGHLIGHT: begin
      MsgData.Channel := Buffer.GetBInt16;
      MsgData.Text := Buffer.GetBStr16();
    end;
  MESSAGE_GAMEMASTER_BROADCAST, MESSAGE_LOGIN, MESSAGE_ADMIN, MESSAGE_GAME, MESSAGE_FAILURE, MESSAGE_LOOK, MESSAGE_STATUS, MESSAGE_LOOT,
    MESSAGE_TRADE_NPC, MESSAGE_GUILD, MESSAGE_PARTY_MANAGEMENT, MESSAGE_PARTY, MESSAGE_HOTKEY_USE, MESSAGE_MARKET,
    MESSAGE_REPORT, MESSAGE_MANA: begin
      MsgData.Text := Buffer.GetBStr16();
    end;
  MESSAGE_DAMAGE_DEALED, MESSAGE_DAMAGE_RECEIVED, MESSAGE_DAMAGE_OTHERS: begin
  ...
  MESSAGE_HEAL, MESSAGE_EXP, MESSAGE_HEAL_OTHERS, MESSAGE_EXP_OTHERS: begin
      if AdrSelected >= TibiaVer1036 then begin
        MsgData.Position := GetPos;
        MsgData.HPDelta1 := Buffer.GetBInt32();
        MsgData.HPColor1 := Buffer.GetBInt8();
...
begin
  ...
  Result.Count := 0;
  if (Result.ID >= TibiaMinItems) and (Result.ID <= TibiaLastItem) then begin
...
begin
  Result.ID := Buffer.GetBUInt16;
  if BInRange(Result.ID, TibiaMinItems, TibiaLastItem) and (idfHasExtra in TibiaItems[Result.ID].DatFlags) then
    Result.Count := BInt8(Buffer.GetBInt8)
  else
    Result.Count := 0;
end;
