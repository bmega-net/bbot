...
function FillGradient(DC: HDC; ARect: TRect; ColorCount: BInt32;
  StartColor, EndColor: TColor; ADirection: TGradientDirection): BBool;
...
function GambitBox(Caption, Intro, Params: BStr; HaveReturn: boolean;
  var Values: BStrArray): boolean;
...
function IntIn(Value: BUInt32; const Values: array of BUInt32)
  : boolean; overload;
...
procedure BListDrawItem(Canvas: TCanvas; Index: BInt32; Selected: BBool;
  Rect: TRect; A: BStr); overload;

procedure BListDrawItem(Canvas: TCanvas; Index: BInt32; Selected: BBool;
  Rect: TRect; A, B: BStr); overload;
...
procedure IterateComponentControls(const AComponent: TComponent;
  const AProc: BUnaryProc<TComponent>);

procedure BListboxKeyDown(const Sender: TObject; var Key: Word;
...
function InsertFrame(const AParent: TPanel;
  const AFrameClass: TFrameClass): TFrame;
  ...
  begin
    ...
    begin
      case Key of
        ...
        VK_DOWN:
          begin
            if Selected < (Lst.Items.Count - 1) then
            begin
              Lst.Items.Exchange(Selected + 1, Selected);
              Lst.ItemIndex := Selected;
            end;
...
begin
  ...
  FillGradient(Canvas.Handle, Rect, 256, frmGradientA, frmGradientB,
    gdVertical);
  Rect.Bottom := Rect.Bottom + hMid;
  Rect.Top := Rect.Top + hMid;
  FillGradient(Canvas.Handle, Rect, 256, frmGradientB, frmGradientA,
    gdVertical);
end;
...
begin
  ...
  begin
    Brush := CreateSolidBrush(RGB(StartRGB[0] + Round((I + 1) * RGBKoef[0]),
      StartRGB[1] + Round((I + 1) * RGBKoef[1]),
      StartRGB[2] + Round((I + 1) * RGBKoef[2])));
    case ADirection of
      gdHorizontal:
        SetRect(ColorRect, Round(RectOffset * I), 0, Round(RectOffset * (I + 1)
          ), AreaHeight);
      gdVertical:
...
begin
  ...
  CreateProcessA(nil, @lCmd[1], nil, nil, False, CREATE_NEW_PROCESS_GROUP, nil,
    @lPath[1], SUInfo, ProcInfo);
...
begin
  Result := '';
  if AnsiSameText(SaveOrLoad, 'load') then
  begin
...
function GambitBox(Caption, Intro, Params: BStr; HaveReturn: boolean;
  var Values: BStrArray): boolean;
  function FastButtonCreate(Caption: BStr; X, Y, Width, Height: BInt32;
    Parent: TWinControl; Kind: TBitBtnKind): TBitBtn;
  begin
  ...
  function FastLabelCreate(Caption: BStr; X, Y: BInt32;
    Parent: TWinControl): TLabel;
  begin
  ...
  function FastMemoCreate(Name: BStr; Y: BInt32; Parent: TWinControl;
    Default: BStr = ''): TEdit;
  begin
...
begin
  ...
  Lst[High(Lst)] := FastMemoCreate('CondRet', Y - 3, F,
    IfThen(not HaveReturn, '', '=1'));
  Inc(Y, 25);
  Lst[High(Lst) - 3] := FastLabelCreate
    ('E.g. =X >X >=X <X <=X <>X (1 is True, 0 is False)', 8, Y, F);
  Inc(Y, 25);
  if not HaveReturn then
    Dec(Y, 2000 + 19 + 25 + 25);
...
begin
  ...
  if length(s) = 1 then
    s := '0' + s;
  Result := IfThen((ZD > 0), D + 'd', '') + IfThen((ZH > 0), H + 'h', '') +
    IfThen((ZM > 0), M + 'm', '') + s + 's';
end;
...
begin
  ...
  Info.Flags := 0;
  try
    RaiseException($406D1388, 0, SizeOf(Info) div SizeOf(LongWord),
      PUINT_PTR(@Info));
  except
  end;
end;
...
procedure PlayWav(const FileName: BStr; const ALoop: BBool);
begin
  if not sndPlaySoundA(BPChar(FileName), BIf(ALoop, SND_ASYNC or SND_LOOP,
    SND_ASYNC)) then
    SysUtils.Beep;
end;
...
begin
  case AKeyCode of
    ...
    $05:
      Result := 'X1 mouse button';
    $06:
      Result := 'X2 mouse button';
    $08:
      Result := 'BACKSPACE';
    $09:
      Result := 'TAB';
    ...
    $5B:
      Result := 'Left Windows';
    $5C:
      Result := 'Right Windows';
    $5D:
      Result := 'Applications';
    $5F:
      Result := 'Computer Sleep';
    $60:
      Result := 'Numeric 0';
    $61:
      Result := 'Numeric 1';
    ...
    $B2:
      Result := 'Stop Media';
    $B3:
      Result := 'Play/Pause Media';
    $B4:
      Result := 'Start Mail';
    $B5:
      Result := 'Select Media';
    $B6:
      Result := 'Start Application 1';
    $B7:
      Result := 'Start Application 2';
    $F6:
      Result := 'Attn';
    $F7:
      Result := 'CrSel';
...
begin
  Res.First := 0;
  Res.Second := [];
  if BStrSplit(R, ' + ', AKeyName) > 1 then
  begin
    KeyName := R[High(R)];
    ...
    KeyName := AKeyName;
  for I := 0 to $FF do
    if BStrEqual(KeyName, KeyCodeToName(I)) then
    begin
      Res.First := I;
      Exit(Res);
    end;
...
procedure TestKeyTool(const AName: BStr; const AKey: BInt16;
...
begin
  ...
  TestKeyTool('Ctrl + Shift + Alt + Numpad 1', VK_NUMPAD1,
