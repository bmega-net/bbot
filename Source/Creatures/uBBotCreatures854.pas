...
type
  TBBotCreatures854 = class(TBBotCreatures)
  private
    Buffer: BPtr;
  protected
...
type
  ...
  PBBotCreature854Buffer = ^TBBotCreature854Buffer;

  TBBotCreatureList854Buffer = array [0 .. TibiaCreatureListSize - 1] of TBBotCreature854Buffer;
  PBBotCreatureList854Buffer = ^TBBotCreatureList854Buffer;

  { TBBotCreature854 }
  TBBotCreature854 = class(TBBotCreature)
  private
    Buffer: PBBotCreature854Buffer;
  ...
  public
    constructor Create(AIndex: BInt32; ABuffer: PBBotCreature854Buffer; ACreatures: TBBotCreatures);
  end;

function TBBotCreature854.GetID: BUInt32;
begin
  Result := Buffer^.ID;
end;

function TBBotCreature854.GetName: BStr;
begin
  Result := BStr(BPChar(@Buffer^.Name[0]));
end;

function TBBotCreature854.GetPosition: BPos;
begin
  Result.X := Buffer^.X;
  Result.Y := Buffer^.Y;
  Result.Z := Buffer^.Z;
...
function TBBotCreature854.GetScreen: TPoint;
begin
  Result.X := Buffer^.ScreenX;
  Result.Y := Buffer^.ScreenY;
end;

function TBBotCreature854.GetDirection: TTibiaDirection;
begin
  if BInRange(Buffer^.Direction, Ord(tdNorth), Ord(tdNorthWest)) then
    Result := TTibiaDirection(Buffer^.Direction)
  else
    raise BException.Create('Unable to load creature direction in 854');
end;

function TBBotCreature854.GetHealth: BInt32;
begin
  Result := Buffer^.HP;
end;

function TBBotCreature854.GetSpeed: BInt32;
begin
  Result := Buffer^.Speed;
end;

function TBBotCreature854.GetSkull: TTibiaSkull;
begin
  if BInRange(Buffer^.Skull, Ord(SkullFirst), Ord(SkullLast)) then
    Result := TTibiaSkull(Buffer^.Skull)
  else
    raise BException.Create('Unable to load creature skull in 854');
end;

function TBBotCreature854.GetParty: TTibiaParty;
begin
  Result.Load(Buffer^.Party);
end;

function TBBotCreature854.GetWar: TTibiaWar;
begin

  if BInRange(Buffer^.War, Ord(WarNone), Ord(WarUnInvolved)) then
    Result := TTibiaWar(Buffer^.War)
  else
    raise BException.Create('Unable to load creature war in 854');
end;

procedure TBBotCreature854.SetLightColor(const Value: BInt32);
begin
  Creatures.Write(Index, GetOffsetLightColor, @Value, 4);
end;

procedure TBBotCreature854.SetLightIntensity(const Value: BInt32);
begin
  Creatures.Write(Index, GetOffsetLightIntensity, @Value, 4);
end;

procedure TBBotCreature854.SetOutfit(const Value: TTibiaOutfit);
var
...
procedure TBBotCreature854.SetWalking(const Value: BBool);
var
  IntValue: BInt32;
begin
...
function TBBotCreature854.GetCreatureKind: TTibiaCreatureKind;
begin
  Result := CreaturePlayer;
end;

function TBBotCreature854.GetNPCKind: TTibiaNPCKind;
begin
  Result := NPCNone;
end;

constructor TBBotCreature854.Create(AIndex: BInt32; ABuffer: PBBotCreature854Buffer; ACreatures: TBBotCreatures);
begin
...
function TBBotCreature854.GetBlackSquareTime: BUInt32;
begin
  Result := Buffer^.BlackSquare;
end;

function TBBotCreature854.GetGroupOnline: BInt32;
begin
  Result := 0;
end;

function TBBotCreature854.GetSquareVisible: BBool;
begin
  Result := False;
end;

function TBBotCreature854.GetSquareRed: BInt32;
begin
  Result := 0;
end;

function TBBotCreature854.GetSquareGreen: BInt32;
begin
  Result := 0;
end;

function TBBotCreature854.GetSquareBlue: BInt32;
begin
  Result := 0;
end;

function TBBotCreature854.GetIsSelf: BBool;
begin
  Result := GetID() = Creatures.PlayerID;
end;

function TBBotCreature854.GetIsPlayer: BBool;
begin
  Result := IsPlayerID(GetID());
end;

function TBBotCreature854.GetIsNPC: BBool;
begin
  Result := not GetIsPlayer();
end;

function TBBotCreature854.GetIsTarget: BBool;
begin
  Result := GetID() = Creatures.TargetID;
end;

function TBBotCreature854.GetIsVisible: BBool;
begin
  Result := Buffer^.IsVisible = 1;
end;

function TBBotCreature854.GetWalking: BBool;
begin
  Result := Buffer^.Walking = 1;
end;

function TBBotCreature854.GetOutfit: TTibiaOutfit;
begin
...
function TBBotCreature854.GetLightIntensity: BInt32;
begin
  Result := Buffer^.Light;
end;

function TBBotCreature854.GetLightColor: BInt32;
begin
  Result := Buffer^.LightColor;
end;

function TBBotCreature854.GetOffsetLightColor: BInt32;
begin
  Result := BInt32(@Buffer^.LightColor) - BInt32(@Buffer^.ID);
end;

function TBBotCreature854.GetOffsetLightIntensity: BInt32;
begin
  Result := BInt32(@Buffer^.Light) - BInt32(@Buffer^.ID);
end;

function TBBotCreature854.GetOffsetOutfit: BInt32;
begin
  Result := BInt32(@Buffer^.Outfit) - BInt32(@Buffer^.ID);
end;

function TBBotCreature854.GetOffsetWalking: BInt32;
begin
...
procedure TBBotCreatures854.Update;
begin
  TibiaProcess.Read(TibiaAddresses.AdrBattle, GetBufferSize, Buffer);
end;

function TBBotCreatures854.GetPlayerID: BUInt32;
begin
  Result := Me.ID;
end;

function TBBotCreatures854.GetTargetID: BUInt32;
begin
  Result := Me.TargetID;
end;

function TBBotCreatures854.GetCreatureBufferSize: BUInt32;
begin
  Result := SizeOf(TBBotCreature854Buffer);
end;

function TBBotCreatures854.GetCreatureBufferCount: BUInt32;
begin
  Result := TibiaCreatureListSize;
end;

procedure TBBotCreatures854.Write(ACreature: BUInt32; AOffset: BUInt32; AValue: BPtr; ASize: BUInt32);
var
  Offset: BUInt32;
begin
...
constructor TBBotCreatures854.Create;
var
  I: BUInt32;
  Add: TCreatureIter;
  BufferList: PBBotCreatureList854Buffer;
begin
  inherited Create;
  New(BufferList);
  Buffer := BufferList;
  for I := 0 to GetCreatureBufferCount - 1 do begin
    ...
    Add^.First := False;
    Add^.Second := TBBotCreature854.Create(I, @BufferList[I], Self);
  end;
end;

destructor TBBotCreatures854.Destroy;
begin
  Dispose(PBBotCreatureList854Buffer(Buffer));
  inherited;
end;
...
type
  TBBotCreatures854Mock = class(TBBotCreatures)
  private
    Buffer: BPtr;
  protected
    procedure Update; override;
  ...
  BCreatures854TestCase = class(TTestCase)
  private
    Creatures: TBBotCreatures;
  protected
  ...
  { TTibiaCreatures854Mock }

procedure TBBotCreatures854Mock.Update;
begin

end;

function TBBotCreatures854Mock.GetPlayerID: BUInt32;
begin
  Result := MockSelfID;
end;

function TBBotCreatures854Mock.GetTargetID: BUInt32;
begin
  Result := MockTargetID;
end;

function TBBotCreatures854Mock.GetCreatureBufferSize: BUInt32;
begin
  Result := SizeOf(TBBotCreature854Buffer);
end;

function TBBotCreatures854Mock.GetCreatureBufferCount: BUInt32;
begin
  Result := TibiaCreatureListSize;
end;

procedure TBBotCreatures854Mock.Write(ACreature: BUInt32; AOffset: BUInt32; AValue: BPtr; ASize: BUInt32);
begin
  Move(AValue^, BPtr(BUInt32(Buffer) + GetCreatureOffset(ACreature) + AOffset)^, ASize);
end;

constructor TBBotCreatures854Mock.Create;
var
  Creature, Player, Target: PBBotCreature854Buffer;
  I: BUInt32;
  Add: TCreatureIter;
  BufferList: PBBotCreatureList854Buffer;
begin
...
destructor TBBotCreatures854Mock.Destroy;
begin
  Dispose(PBBotCreatureList854Buffer(Buffer));
  inherited;
end;

{ BCreatures854TestCase }

procedure BCreatures854TestCase.SetUp;
begin
  Creatures := TBBotCreatures854Mock.Create;
  Creatures.Reload;
end;

procedure BCreatures854TestCase.TearDown;
begin
  Creatures.Free;
end;

procedure BCreatures854TestCase.TestCreatureCount;
var
  Count: BUInt32;
begin
  Count := 0;
...
procedure BCreatures854TestCase.TestTarget;
begin
  Check(Creatures.Target <> nil);
end;

procedure BCreatures854TestCase.TestSelf;
begin
  Check(Creatures.Player <> nil);
end;

procedure BCreatures854TestCase.TestTargetDistance;
begin
  Check(Creatures.Player <> nil);
  Check(Creatures.Target <> nil);
...
procedure BCreatures854TestCase.TestSelfLight;
begin
  Check(Creatures.Player <> nil);
...
procedure BCreatures854TestCase.TestSelfOutfit;
var
  Outfit: TTibiaOutfit;
begin
  Check(Creatures.Player <> nil);
  ...
  CheckEquals(0, Creatures.Player.Outfit.Mount, 'Version 854 does not have outfit.mount');
end;

procedure BCreatures854TestCase.TestSelfWalking;
begin
  Check(Creatures.Player <> nil);
...
TestFramework.RegisterTest('BCreatures854', BCreatures854TestCase.Suite);
{$ENDIF}
