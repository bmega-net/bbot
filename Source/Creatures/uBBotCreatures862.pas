...
type
  TBBotCreatures862 = class(TBBotCreatures)
  private
    Buffer: BPtr;
  protected
...
type
  ...
  PBBotCreature862Buffer = ^TBBotCreature862Buffer;

  TBBotCreatureList862Buffer = array [0 .. TibiaCreatureListSize - 1] of TBBotCreature862Buffer;
  PBBotCreatureList862Buffer = ^TBBotCreatureList862Buffer;

  { TBBotCreature862 }
  TBBotCreature862 = class(TBBotCreature)
  private
    Buffer: PBBotCreature862Buffer;
  ...
  public
    constructor Create(AIndex: BInt32; ABuffer: PBBotCreature862Buffer; ACreatures: TBBotCreatures);
  end;

function TBBotCreature862.GetID: BUInt32;
begin
  Result := Buffer^.ID;
end;

function TBBotCreature862.GetName: BStr;
begin
  Result := BStr(BPChar(@Buffer^.Name[0]));
end;

function TBBotCreature862.GetPosition: BPos;
begin
  Result.X := Buffer^.X;
  Result.Y := Buffer^.Y;
  Result.Z := Buffer^.Z;
...
function TBBotCreature862.GetScreen: TPoint;
begin
  Result.X := Buffer^.ScreenX;
  Result.Y := Buffer^.ScreenY;
end;

function TBBotCreature862.GetDirection: TTibiaDirection;
begin
  if BInRange(Buffer^.Direction, Ord(tdNorth), Ord(tdNorthWest)) then
    Result := TTibiaDirection(Buffer^.Direction)
  else
    raise BException.Create('Unable to load creature direction in 862');
end;

function TBBotCreature862.GetHealth: BInt32;
begin
  Result := Buffer^.HP;
end;

function TBBotCreature862.GetSpeed: BInt32;
begin
  Result := Buffer^.Speed;
end;

function TBBotCreature862.GetSkull: TTibiaSkull;
begin
  if BInRange(Buffer^.Skull, Ord(SkullFirst), Ord(SkullLast)) then
    Result := TTibiaSkull(Buffer^.Skull)
  else
    raise BException.Create('Unable to load creature skull in 862');
end;

function TBBotCreature862.GetParty: TTibiaParty;
begin
  Result.Load(Buffer^.Party);
end;

function TBBotCreature862.GetWar: TTibiaWar;
begin

  if BInRange(Buffer^.War, Ord(WarNone), Ord(WarUnInvolved)) then
    Result := TTibiaWar(Buffer^.War)
  else
    raise BException.Create('Unable to load creature war in 862');
end;

procedure TBBotCreature862.SetLightColor(const Value: BInt32);
begin
  Creatures.Write(Index, GetOffsetLightColor, @Value, 4);
end;

procedure TBBotCreature862.SetLightIntensity(const Value: BInt32);
begin
  Creatures.Write(Index, GetOffsetLightIntensity, @Value, 4);
end;

procedure TBBotCreature862.SetOutfit(const Value: TTibiaOutfit);
var
...
procedure TBBotCreature862.SetWalking(const Value: BBool);
var
  IntValue: BInt32;
begin
...
function TBBotCreature862.GetCreatureKind: TTibiaCreatureKind;
begin
  Result := CreaturePlayer;
end;

function TBBotCreature862.GetNPCKind: TTibiaNPCKind;
begin
  Result := NPCNone;
end;

constructor TBBotCreature862.Create(AIndex: BInt32; ABuffer: PBBotCreature862Buffer; ACreatures: TBBotCreatures);
begin
...
function TBBotCreature862.GetBlackSquareTime: BUInt32;
begin
  Result := Buffer^.BlackSquare;
end;

function TBBotCreature862.GetGroupOnline: BInt32;
begin
  Result := 0;
end;

function TBBotCreature862.GetSquareVisible: BBool;
begin
  Result := False;
end;

function TBBotCreature862.GetSquareRed: BInt32;
begin
  Result := 0;
end;

function TBBotCreature862.GetSquareGreen: BInt32;
begin
  Result := 0;
end;

function TBBotCreature862.GetSquareBlue: BInt32;
begin
  Result := 0;
end;

function TBBotCreature862.GetIsSelf: BBool;
begin
  Result := GetID() = Creatures.PlayerID;
end;

function TBBotCreature862.GetIsPlayer: BBool;
begin
  Result := IsPlayerID(GetID());
end;

function TBBotCreature862.GetIsNPC: BBool;
begin
  Result := not GetIsPlayer();
end;

function TBBotCreature862.GetIsTarget: BBool;
begin
  Result := GetID() = Creatures.TargetID;
end;

function TBBotCreature862.GetIsVisible: BBool;
begin
  Result := Buffer^.IsVisible = 1;
end;

function TBBotCreature862.GetWalking: BBool;
begin
  Result := Buffer^.Walking = 1;
end;

function TBBotCreature862.GetOutfit: TTibiaOutfit;
begin
...
function TBBotCreature862.GetLightIntensity: BInt32;
begin
  Result := Buffer^.Light;
end;

function TBBotCreature862.GetLightColor: BInt32;
begin
  Result := Buffer^.LightColor;
end;

function TBBotCreature862.GetOffsetLightColor: BInt32;
begin
  Result := BInt32(@Buffer^.LightColor) - BInt32(@Buffer^.ID);
end;

function TBBotCreature862.GetOffsetLightIntensity: BInt32;
begin
  Result := BInt32(@Buffer^.Light) - BInt32(@Buffer^.ID);
end;

function TBBotCreature862.GetOffsetOutfit: BInt32;
begin
  Result := BInt32(@Buffer^.Outfit) - BInt32(@Buffer^.ID);
end;

function TBBotCreature862.GetOffsetWalking: BInt32;
begin
...
procedure TBBotCreatures862.Update;
begin
  TibiaProcess.Read(TibiaAddresses.AdrBattle, GetBufferSize, Buffer);
end;

function TBBotCreatures862.GetPlayerID: BUInt32;
begin
  Result := Me.ID;
end;

function TBBotCreatures862.GetTargetID: BUInt32;
begin
  Result := Me.TargetID;
end;

function TBBotCreatures862.GetCreatureBufferSize: BUInt32;
begin
  Result := SizeOf(TBBotCreature862Buffer);
end;

function TBBotCreatures862.GetCreatureBufferCount: BUInt32;
begin
  Result := TibiaCreatureListSize;
end;

procedure TBBotCreatures862.Write(ACreature: BUInt32; AOffset: BUInt32; AValue: BPtr; ASize: BUInt32);
var
  Offset: BUInt32;
begin
...
constructor TBBotCreatures862.Create;
var
  I: BUInt32;
  Add: TCreatureIter;
  BufferList: PBBotCreatureList862Buffer;
begin
  inherited Create;
  New(BufferList);
  Buffer := BufferList;
  for I := 0 to GetCreatureBufferCount - 1 do begin
    ...
    Add^.First := False;
    Add^.Second := TBBotCreature862.Create(I, @BufferList[I], Self);
  end;
end;

destructor TBBotCreatures862.Destroy;
begin
  Dispose(PBBotCreatureList862Buffer(Buffer));
  inherited;
end;
...
type
  TBBotCreatures862Mock = class(TBBotCreatures)
  private
    Buffer: BPtr;
  protected
    procedure Update; override;
  ...
  BCreatures862TestCase = class(TTestCase)
  private
    Creatures: TBBotCreatures;
  protected
  ...
  { TTibiaCreatures862Mock }

procedure TBBotCreatures862Mock.Update;
begin

end;

function TBBotCreatures862Mock.GetPlayerID: BUInt32;
begin
  Result := MockSelfID;
end;

function TBBotCreatures862Mock.GetTargetID: BUInt32;
begin
  Result := MockTargetID;
end;

function TBBotCreatures862Mock.GetCreatureBufferSize: BUInt32;
begin
  Result := SizeOf(TBBotCreature862Buffer);
end;

function TBBotCreatures862Mock.GetCreatureBufferCount: BUInt32;
begin
  Result := TibiaCreatureListSize;
end;

procedure TBBotCreatures862Mock.Write(ACreature: BUInt32; AOffset: BUInt32; AValue: BPtr; ASize: BUInt32);
begin
  Move(AValue^, BPtr(BUInt32(Buffer) + GetCreatureOffset(ACreature) + AOffset)^, ASize);
end;

constructor TBBotCreatures862Mock.Create;
var
  Creature, Player, Target: PBBotCreature862Buffer;
  I: BUInt32;
  Add: TCreatureIter;
  BufferList: PBBotCreatureList862Buffer;
begin
...
destructor TBBotCreatures862Mock.Destroy;
begin
  Dispose(PBBotCreatureList862Buffer(Buffer));
  inherited;
end;

{ BCreatures862TestCase }

procedure BCreatures862TestCase.SetUp;
begin
  Creatures := TBBotCreatures862Mock.Create;
  Creatures.Reload;
end;

procedure BCreatures862TestCase.TearDown;
begin
  Creatures.Free;
end;

procedure BCreatures862TestCase.TestCreatureCount;
var
  Count: BUInt32;
begin
  Count := 0;
...
procedure BCreatures862TestCase.TestTarget;
begin
  Check(Creatures.Target <> nil);
end;

procedure BCreatures862TestCase.TestSelf;
begin
  Check(Creatures.Player <> nil);
end;

procedure BCreatures862TestCase.TestTargetDistance;
begin
  Check(Creatures.Player <> nil);
  Check(Creatures.Target <> nil);
...
procedure BCreatures862TestCase.TestSelfLight;
begin
  Check(Creatures.Player <> nil);
...
procedure BCreatures862TestCase.TestSelfOutfit;
var
  Outfit: TTibiaOutfit;
begin
  Check(Creatures.Player <> nil);
  ...
  CheckEquals(0, Creatures.Player.Outfit.Mount, 'Version 862 does not have outfit.mount');
end;

procedure BCreatures862TestCase.TestSelfWalking;
begin
  Check(Creatures.Player <> nil);
...
TestFramework.RegisterTest('BCreatures862', BCreatures862TestCase.Suite);
{$ENDIF}
