...
type
  TBBotCreatures990 = class(TBBotCreatures)
  private
    Buffer: BPtr;
  protected
...
type
  ...
  PBBotCreature990Buffer = ^TBBotCreature990Buffer;

  TBBotCreatureList990Buffer = array [0 .. TibiaCreatureListSize - 1] of TBBotCreature990Buffer;
  PBBotCreatureList990Buffer = ^TBBotCreatureList990Buffer;

  { TBBotCreature990 }
  TBBotCreature990 = class(TBBotCreature)
  private
    Buffer: PBBotCreature990Buffer;
  ...
  public
    constructor Create(AIndex: BInt32; ABuffer: PBBotCreature990Buffer; ACreatures: TBBotCreatures);
  end;

function TBBotCreature990.GetID: BUInt32;
begin
  Result := Buffer^.ID;
end;

function TBBotCreature990.GetName: BStr;
begin
  Result := BStr(BPChar(@Buffer^.Name[0]));
end;

function TBBotCreature990.GetPosition: BPos;
begin
  Result.X := Buffer^.X;
  Result.Y := Buffer^.Y;
  Result.Z := Buffer^.Z;
...
function TBBotCreature990.GetScreen: TPoint;
begin
  Result.X := Buffer^.ScreenX;
  Result.Y := Buffer^.ScreenY;
end;

function TBBotCreature990.GetDirection: TTibiaDirection;
begin
  if BInRange(Buffer^.Direction, Ord(tdNorth), Ord(tdNorthWest)) then
    Result := TTibiaDirection(Buffer^.Direction)
  else
    raise BException.Create('Unable to load creature direction in 990');
end;

function TBBotCreature990.GetHealth: BInt32;
begin
  Result := Buffer^.HP;
end;

function TBBotCreature990.GetSpeed: BInt32;
begin
  Result := Buffer^.Speed;
end;

function TBBotCreature990.GetSkull: TTibiaSkull;
begin
  if BInRange(Buffer^.Skull, Ord(SkullFirst), Ord(SkullLast)) then
    Result := TTibiaSkull(Buffer^.Skull)
  else
    raise BException.Create('Unable to load creature skull in 990');
end;

function TBBotCreature990.GetParty: TTibiaParty;
begin
  Result.Load(Buffer^.Party);
end;

function TBBotCreature990.GetWar: TTibiaWar;
begin

  if BInRange(Buffer^.War, Ord(WarNone), Ord(WarUnInvolved)) then
    Result := TTibiaWar(Buffer^.War)
  else
    raise BException.Create('Unable to load creature war in 990');
end;

procedure TBBotCreature990.SetLightColor(const Value: BInt32);
begin
  Creatures.Write(Index, GetOffsetLightColor, @Value, 4);
end;

procedure TBBotCreature990.SetLightIntensity(const Value: BInt32);
begin
  Creatures.Write(Index, GetOffsetLightIntensity, @Value, 4);
end;

procedure TBBotCreature990.SetOutfit(const Value: TTibiaOutfit);
var
...
procedure TBBotCreature990.SetWalking(const Value: BBool);
var
  IntValue: BInt32;
begin
...
function TBBotCreature990.GetCreatureKind: TTibiaCreatureKind;
begin
  if BInRange(Buffer^.CreatureType, Ord(CreaturePlayer), Ord(CreatureNPC)) then
    Result := TTibiaCreatureKind(Buffer^.CreatureType)
  else
    raise BException.Create('Unable to load creature kind in 990');
end;

function TBBotCreature990.GetNPCKind: TTibiaNPCKind;
begin
  Result := NPCNone;
end;

constructor TBBotCreature990.Create(AIndex: BInt32; ABuffer: PBBotCreature990Buffer; ACreatures: TBBotCreatures);
begin
...
function TBBotCreature990.GetBlackSquareTime: BUInt32;
begin
  Result := Buffer^.BlackSquare;
end;

function TBBotCreature990.GetGroupOnline: BInt32;
begin
  Result := Buffer^.GroupOnline;
end;

function TBBotCreature990.GetSquareVisible: BBool;
begin
  Result := Buffer^.Square.Alpha = 1;
end;

function TBBotCreature990.GetSquareRed: BInt32;
begin
  Result := Buffer^.Square.Red;
end;

function TBBotCreature990.GetSquareGreen: BInt32;
begin
  Result := Buffer^.Square.Green;
end;

function TBBotCreature990.GetSquareBlue: BInt32;
begin
  Result := Buffer^.Square.Blue;
end;

function TBBotCreature990.GetIsSelf: BBool;
begin
  Result := GetID() = Creatures.PlayerID;
end;

function TBBotCreature990.GetIsPlayer: BBool;
begin
  Result := IsPlayerID(GetID());
end;

function TBBotCreature990.GetIsNPC: BBool;
begin
  Result := not GetIsPlayer();
end;

function TBBotCreature990.GetIsTarget: BBool;
begin
  Result := GetID() = Creatures.TargetID;
end;

function TBBotCreature990.GetIsVisible: BBool;
begin
  Result := Buffer^.IsVisible = 1;
end;

function TBBotCreature990.GetWalking: BBool;
begin
  Result := Buffer^.Walking = 1;
end;

function TBBotCreature990.GetOutfit: TTibiaOutfit;
begin
...
function TBBotCreature990.GetLightIntensity: BInt32;
begin
  Result := Buffer^.Light;
end;

function TBBotCreature990.GetLightColor: BInt32;
begin
  Result := Buffer^.LightColor;
end;

function TBBotCreature990.GetOffsetLightColor: BInt32;
begin
  Result := BInt32(@Buffer^.LightColor) - BInt32(@Buffer^.ID);
end;

function TBBotCreature990.GetOffsetLightIntensity: BInt32;
begin
  Result := BInt32(@Buffer^.Light) - BInt32(@Buffer^.ID);
end;

function TBBotCreature990.GetOffsetOutfit: BInt32;
begin
  Result := BInt32(@Buffer^.Outfit) - BInt32(@Buffer^.ID);
end;

function TBBotCreature990.GetOffsetWalking: BInt32;
begin
...
procedure TBBotCreatures990.Update;
begin
  TibiaProcess.Read(TibiaAddresses.AdrBattle, GetBufferSize, Buffer);
end;

function TBBotCreatures990.GetPlayerID: BUInt32;
begin
  Result := Me.ID;
end;

function TBBotCreatures990.GetTargetID: BUInt32;
begin
  Result := Me.TargetID;
end;

function TBBotCreatures990.GetCreatureBufferSize: BUInt32;
begin
  Result := SizeOf(TBBotCreature990Buffer);
end;

function TBBotCreatures990.GetCreatureBufferCount: BUInt32;
begin
  Result := TibiaCreatureListSize;
end;

procedure TBBotCreatures990.Write(ACreature: BUInt32; AOffset: BUInt32; AValue: BPtr; ASize: BUInt32);
var
  Offset: BUInt32;
begin
...
constructor TBBotCreatures990.Create;
var
  I: BUInt32;
  Add: TCreatureIter;
  BufferList: PBBotCreatureList990Buffer;
begin
  inherited Create;
  New(BufferList);
  Buffer := BufferList;
  for I := 0 to GetCreatureBufferCount - 1 do begin
    ...
    Add^.First := False;
    Add^.Second := TBBotCreature990.Create(I, @BufferList[I], Self);
  end;
end;

destructor TBBotCreatures990.Destroy;
begin
  Dispose(PBBotCreatureList990Buffer(Buffer));
  inherited;
end;
...
type
  TBBotCreatures990Mock = class(TBBotCreatures)
  private
    Buffer: BPtr;
  protected
    procedure Update; override;
  ...
  BCreatures990TestCase = class(TTestCase)
  private
    Creatures: TBBotCreatures;
  protected
  ...
  { TTibiaCreatures990Mock }

procedure TBBotCreatures990Mock.Update;
begin

end;

function TBBotCreatures990Mock.GetPlayerID: BUInt32;
begin
  Result := MockSelfID;
end;

function TBBotCreatures990Mock.GetTargetID: BUInt32;
begin
  Result := MockTargetID;
end;

function TBBotCreatures990Mock.GetCreatureBufferSize: BUInt32;
begin
  Result := SizeOf(TBBotCreature990Buffer);
end;

function TBBotCreatures990Mock.GetCreatureBufferCount: BUInt32;
begin
  Result := TibiaCreatureListSize;
end;

procedure TBBotCreatures990Mock.Write(ACreature: BUInt32; AOffset: BUInt32; AValue: BPtr; ASize: BUInt32);
begin
  Move(AValue^, BPtr(BUInt32(Buffer) + GetCreatureOffset(ACreature) + AOffset)^, ASize);
end;

constructor TBBotCreatures990Mock.Create;
var
  Creature, Player, Target: PBBotCreature990Buffer;
  I: BUInt32;
  Add: TCreatureIter;
  BufferList: PBBotCreatureList990Buffer;
begin
...
destructor TBBotCreatures990Mock.Destroy;
begin
  Dispose(PBBotCreatureList990Buffer(Buffer));
  inherited;
end;

{ BCreatures990TestCase }

procedure BCreatures990TestCase.SetUp;
begin
  Creatures := TBBotCreatures990Mock.Create;
  Creatures.Reload;
end;

procedure BCreatures990TestCase.TearDown;
begin
  Creatures.Free;
end;

procedure BCreatures990TestCase.TestCreatureCount;
var
  Count: BUInt32;
begin
  Count := 0;
...
procedure BCreatures990TestCase.TestTarget;
begin
  Check(Creatures.Target <> nil);
end;

procedure BCreatures990TestCase.TestSelf;
begin
  Check(Creatures.Player <> nil);
end;

procedure BCreatures990TestCase.TestTargetDistance;
begin
  Check(Creatures.Player <> nil);
  Check(Creatures.Target <> nil);
...
procedure BCreatures990TestCase.TestSelfLight;
begin
  Check(Creatures.Player <> nil);
...
procedure BCreatures990TestCase.TestSelfOutfit;
var
  Outfit: TTibiaOutfit;
begin
  Check(Creatures.Player <> nil);
...
procedure BCreatures990TestCase.TestSelfWalking;
begin
  Check(Creatures.Player <> nil);
...
TestFramework.RegisterTest('BCreatures990', BCreatures990TestCase.Suite);
{$ENDIF}
