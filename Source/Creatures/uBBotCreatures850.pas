...
type
  TBBotCreatures850 = class(TBBotCreatures)
  private
    Buffer: BPtr;
  protected
...
type
  ...
  PBBotCreature850Buffer = ^TBBotCreature850Buffer;

  TBBotCreatureList850Buffer = array [0 .. TibiaCreatureListSize - 1] of TBBotCreature850Buffer;
  PBBotCreatureList850Buffer = ^TBBotCreatureList850Buffer;

  { TBBotCreature850 }
  TBBotCreature850 = class(TBBotCreature)
  private
    Buffer: PBBotCreature850Buffer;
  ...
  public
    constructor Create(AIndex: BInt32; ABuffer: PBBotCreature850Buffer; ACreatures: TBBotCreatures);
  end;

function TBBotCreature850.GetID: BUInt32;
begin
  Result := Buffer^.ID;
end;

function TBBotCreature850.GetName: BStr;
begin
  Result := BStr(BPChar(@Buffer^.Name[0]));
end;

function TBBotCreature850.GetPosition: BPos;
begin
  Result.X := Buffer^.X;
  Result.Y := Buffer^.Y;
  Result.Z := Buffer^.Z;
...
function TBBotCreature850.GetScreen: TPoint;
begin
  Result.X := Buffer^.ScreenX;
  Result.Y := Buffer^.ScreenY;
end;

function TBBotCreature850.GetDirection: TTibiaDirection;
begin
  if BInRange(Buffer^.Direction, Ord(tdNorth), Ord(tdNorthWest)) then
    Result := TTibiaDirection(Buffer^.Direction)
  else
    raise BException.Create('Unable to load creature direction in 850');
end;

function TBBotCreature850.GetHealth: BInt32;
begin
  Result := Buffer^.HP;
end;

function TBBotCreature850.GetSpeed: BInt32;
begin
  Result := Buffer^.Speed;
end;

function TBBotCreature850.GetSkull: TTibiaSkull;
begin
  if BInRange(Buffer^.Skull, Ord(SkullFirst), Ord(SkullLast)) then
    Result := TTibiaSkull(Buffer^.Skull)
  else
    raise BException.Create('Unable to load creature skull in 850');
end;

function TBBotCreature850.GetParty: TTibiaParty;
begin
  Result.Load(Buffer^.Party);
end;

function TBBotCreature850.GetWar: TTibiaWar;
begin
  Result := WarNone;
end;

procedure TBBotCreature850.SetLightColor(const Value: BInt32);
begin
  Creatures.Write(Index, GetOffsetLightColor, @Value, 4);
end;

procedure TBBotCreature850.SetLightIntensity(const Value: BInt32);
begin
  Creatures.Write(Index, GetOffsetLightIntensity, @Value, 4);
end;

procedure TBBotCreature850.SetOutfit(const Value: TTibiaOutfit);
var
...
procedure TBBotCreature850.SetWalking(const Value: BBool);
var
  IntValue: BInt32;
begin
...
function TBBotCreature850.GetCreatureKind: TTibiaCreatureKind;
begin
  Result := CreaturePlayer;
end;

function TBBotCreature850.GetNPCKind: TTibiaNPCKind;
begin
  Result := NPCNone;
end;

constructor TBBotCreature850.Create(AIndex: BInt32; ABuffer: PBBotCreature850Buffer; ACreatures: TBBotCreatures);
begin
...
function TBBotCreature850.GetBlackSquareTime: BUInt32;
begin
  Result := Buffer^.BlackSquare;
end;

function TBBotCreature850.GetGroupOnline: BInt32;
begin
  Result := 0;
end;

function TBBotCreature850.GetSquareVisible: BBool;
begin
  Result := False;
end;

function TBBotCreature850.GetSquareRed: BInt32;
begin
  Result := 0;
end;

function TBBotCreature850.GetSquareGreen: BInt32;
begin
  Result := 0;
end;

function TBBotCreature850.GetSquareBlue: BInt32;
begin
  Result := 0;
end;

function TBBotCreature850.GetIsSelf: BBool;
begin
  Result := GetID() = Creatures.PlayerID;
end;

function TBBotCreature850.GetIsPlayer: BBool;
begin
  Result := IsPlayerID(GetID());
end;

function TBBotCreature850.GetIsNPC: BBool;
begin
  Result := not GetIsPlayer();
end;

function TBBotCreature850.GetIsTarget: BBool;
begin
  Result := GetID() = Creatures.TargetID;
end;

function TBBotCreature850.GetIsVisible: BBool;
begin
  Result := Buffer^.IsVisible = 1;
end;

function TBBotCreature850.GetWalking: BBool;
begin
  Result := Buffer^.Walking = 1;
end;

function TBBotCreature850.GetOutfit: TTibiaOutfit;
begin
...
function TBBotCreature850.GetLightIntensity: BInt32;
begin
  Result := Buffer^.Light;
end;

function TBBotCreature850.GetLightColor: BInt32;
begin
  Result := Buffer^.LightColor;
end;

function TBBotCreature850.GetOffsetLightColor: BInt32;
begin
  Result := BInt32(@Buffer^.LightColor) - BInt32(@Buffer^.ID);
end;

function TBBotCreature850.GetOffsetLightIntensity: BInt32;
begin
  Result := BInt32(@Buffer^.Light) - BInt32(@Buffer^.ID);
end;

function TBBotCreature850.GetOffsetOutfit: BInt32;
begin
  Result := BInt32(@Buffer^.Outfit) - BInt32(@Buffer^.ID);
end;

function TBBotCreature850.GetOffsetWalking: BInt32;
begin
...
procedure TBBotCreatures850.Update;
begin
  TibiaProcess.Read(TibiaAddresses.AdrBattle, GetBufferSize, Buffer);
end;

function TBBotCreatures850.GetPlayerID: BUInt32;
begin
  Result := Me.ID;
end;

function TBBotCreatures850.GetTargetID: BUInt32;
begin
  Result := Me.TargetID;
end;

function TBBotCreatures850.GetCreatureBufferSize: BUInt32;
begin
  Result := SizeOf(TBBotCreature850Buffer);
end;

function TBBotCreatures850.GetCreatureBufferCount: BUInt32;
begin
  Result := TibiaCreatureListSize;
end;

procedure TBBotCreatures850.Write(ACreature: BUInt32; AOffset: BUInt32; AValue: BPtr; ASize: BUInt32);
var
  Offset: BUInt32;
begin
...
constructor TBBotCreatures850.Create;
var
  I: BUInt32;
  Add: TCreatureIter;
  BufferList: PBBotCreatureList850Buffer;
begin
  inherited Create;
  New(BufferList);
  Buffer := BufferList;
  for I := 0 to GetCreatureBufferCount - 1 do begin
    ...
    Add^.First := False;
    Add^.Second := TBBotCreature850.Create(I, @BufferList[I], Self);
  end;
end;

destructor TBBotCreatures850.Destroy;
begin
  Dispose(PBBotCreatureList850Buffer(Buffer));
  inherited;
end;
...
type
  TBBotCreatures850Mock = class(TBBotCreatures)
  private
    Buffer: BPtr;
  protected
    procedure Update; override;
  ...
  BCreatures850TestCase = class(TTestCase)
  private
    Creatures: TBBotCreatures;
  protected
  ...
  { TTibiaCreatures850Mock }

procedure TBBotCreatures850Mock.Update;
begin

end;

function TBBotCreatures850Mock.GetPlayerID: BUInt32;
begin
  Result := MockSelfID;
end;

function TBBotCreatures850Mock.GetTargetID: BUInt32;
begin
  Result := MockTargetID;
end;

function TBBotCreatures850Mock.GetCreatureBufferSize: BUInt32;
begin
  Result := SizeOf(TBBotCreature850Buffer);
end;

function TBBotCreatures850Mock.GetCreatureBufferCount: BUInt32;
begin
  Result := TibiaCreatureListSize;
end;

procedure TBBotCreatures850Mock.Write(ACreature: BUInt32; AOffset: BUInt32; AValue: BPtr; ASize: BUInt32);
begin
  Move(AValue^, BPtr(BUInt32(Buffer) + GetCreatureOffset(ACreature) + AOffset)^, ASize);
end;

constructor TBBotCreatures850Mock.Create;
var
  Creature, Player, Target: PBBotCreature850Buffer;
  I: BUInt32;
  Add: TCreatureIter;
  BufferList: PBBotCreatureList850Buffer;
begin
...
destructor TBBotCreatures850Mock.Destroy;
begin
  Dispose(PBBotCreatureList850Buffer(Buffer));
  inherited;
end;

{ BCreatures850TestCase }

procedure BCreatures850TestCase.SetUp;
begin
  Creatures := TBBotCreatures850Mock.Create;
  Creatures.Reload;
end;

procedure BCreatures850TestCase.TearDown;
begin
  Creatures.Free;
end;

procedure BCreatures850TestCase.TestCreatureCount;
var
  Count: BUInt32;
begin
  Count := 0;
...
procedure BCreatures850TestCase.TestTarget;
begin
  Check(Creatures.Target <> nil);
end;

procedure BCreatures850TestCase.TestSelf;
begin
  Check(Creatures.Player <> nil);
end;

procedure BCreatures850TestCase.TestTargetDistance;
begin
  Check(Creatures.Player <> nil);
  Check(Creatures.Target <> nil);
...
procedure BCreatures850TestCase.TestSelfLight;
begin
  Check(Creatures.Player <> nil);
...
procedure BCreatures850TestCase.TestSelfOutfit;
var
  Outfit: TTibiaOutfit;
begin
  Check(Creatures.Player <> nil);
  ...
  CheckEquals(0, Creatures.Player.Outfit.Mount, 'Version 850 does not have outfit.mount');
end;

procedure BCreatures850TestCase.TestSelfWalking;
begin
  Check(Creatures.Player <> nil);
...
TestFramework.RegisterTest('BCreatures850', BCreatures850TestCase.Suite);
{$ENDIF}
