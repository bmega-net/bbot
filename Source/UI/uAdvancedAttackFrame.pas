...
type
  TAdvancedAttackFrame = class(TFrame)
    ...
    procedure AdvancedAttackCreaturesDrawItem(Control: TWinControl;
      Index: Integer; Rect: TRect; State: TOwnerDrawState);
    procedure AdvancedAttackTriggerKindClick(Sender: TObject);
    procedure AdvancedAttackListDrawItem(Control: TWinControl; Index: Integer;
    ...
    procedure AdvancedAttackListKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure RefreshTargetCreaturesTimer(Sender: TObject);
    procedure AdvancedAttackTargetAreaLimitScreenDrawItem(Control: TWinControl;
      Index: Integer; Rect: TRect; State: TOwnerDrawState);
  private
...
begin
  AdvancedAttackCreatures.Checked[AdvancedAttackCreatures.Items.Count -
    1] := False;
end;
...
var
  Name: BStr;
begin
  if AdvancedAttackCreatures.ItemIndex = AdvancedAttackCreatures.Items.Count - 1
  then
  begin
    Name := InputBox('New creature name', 'Add creature', '');
    if Name <> '' then
    begin
      AdvancedAttackCreatures.Items.Insert(AdvancedAttackCreatures.Items.Count
        - 1, Name);
    end;
  end;
end;

procedure TAdvancedAttackFrame.AdvancedAttackCreaturesDrawItem
...
begin
  ...
  if BStrStartSensitive(B, '%') then
  begin
    A := BStrCopy(B, 2, Length(B) - 1);
    B := '';
  end;
  BListDrawItem(AdvancedAttackCreatures.Canvas, Index, odSelected in State,
    Rect, A, B);
end;
...
begin
  ...
  Data := TJson.Create;
  Data.Parse(BStrRight(AdvancedAttackList.Items[AdvancedAttackList.ItemIndex],
    StrAdvancedAttackDataDelimiter));
  ...
  begin
    ...
    AdvancedAttackSelfAreaMinCreatures.Text :=
      BToStr(Data.Get('minCreatures').AsInteger);
  ...
  begin
    ...
    AdvancedAttackTargetAreaMinCreatures.Text :=
      BToStr(Data.Get('minCreatures').AsInteger);
    AdvancedAttackTargetAreaRadius.Text := BToStr(Data.Get('radius').AsInteger);
    AdvancedAttackTargetAreaLimitScreen.ItemIndex := Data.Get('limitScreen')
      .AsInteger;
  end
  else if BStrEqual(Kind, 'Waver') then
  begin
    AdvancedAttackTriggerKind.ItemIndex := 3;
    AdvancedAttackWaveMinCreatures.Text :=
      BToStr(Data.Get('minCreatures').AsInteger);
...
procedure TAdvancedAttackFrame.AdvancedAttackListDrawItem(Control: TWinControl;
  Index: Integer; Rect: TRect; State: TOwnerDrawState);
var
  Name: BStr;
begin
  Name := BStrLeft(AdvancedAttackList.Items[Index],
    StrAdvancedAttackDataDelimiter);
  BListDrawItem(AdvancedAttackList.Canvas, Index, odSelected in State,
    Rect, Name);
end;

procedure TAdvancedAttackFrame.AdvancedAttackListKeyDown(Sender: TObject;
...
var
  ...
  I: BInt32;
begin
  if Length(AdvancedAttackName.Text) < 3 then
  begin
    ShowMessage('Please set a name!');
    Exit;
  end;
  if (Length(AdvancedAttackAction.Text) = 0) or
    (AdvancedAttackAction.Text = StrManageAttackSequen) then
  begin
    ShowMessage('Please select a action!');
    Exit;
  end;
  if not ContainsCheckedCreature() then
  begin
  ...
  case AdvancedAttackTriggerKind.ItemIndex of
    ...
    1:
      begin // Area Spell
        ...
        Data.Put('minCreatures',
          BStrTo32(AdvancedAttackSelfAreaMinCreatures.Text));
    ...
    2:
      begin // Area Shooter
        ...
        Data.Put('minCreatures',
          BStrTo32(AdvancedAttackTargetAreaMinCreatures.Text));
  ...
  Code := AdvancedAttackName.Text + StrAdvancedAttackDataDelimiter +
    Data.Stringify;
  Data.Free;
  for I := 0 to AdvancedAttackList.Items.Count - 1 do
    if BStrEqual(BStrLeft(AdvancedAttackList.Items[I], '@Data='),
      AdvancedAttackName.Text) then
    begin
...
begin
  ...
  begin
    ...
    try
      ...
      for I := 0 to AdvancedAttackList.Count - 1 do
      begin
        Json.Parse(BStrRight(AdvancedAttackList.Items[I],
          StrAdvancedAttackDataDelimiter));
...
procedure TAdvancedAttackFrame.AdvancedAttackTargetAreaLimitScreenDrawItem
  (Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
var
  A, B: BStr;
begin
  BStrSplit(AdvancedAttackTargetAreaLimitScreen.Items[Index], ' ', A, B);
  BListDrawItem(AdvancedAttackTargetAreaLimitScreen.Canvas, Index,
    odSelected in State, Rect, A, B);
end;
...
begin
  WaveFormatDesigner := TAdvancedAttackWaveFormatDesignerFrame
    (InsertFrame(WaveFormatDesignerPlaceholder,
    ...
    BBot.Creatures.KnownCreatureNames.ForEach(
      ...
      begin
        if AdvancedAttackCreatures.Items.IndexOf(AIt^) = -1 then
          AdvancedAttackCreatures.Items.Insert
            (AdvancedAttackCreatures.Items.Count - 1, AIt^);
      end);
end;
