...
type
  ...
  TAdvancedAttackWaveFormatDesignerFrame = class(TFrame)
    ...
    procedure MainBufferMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure AutoRedrawTimer(Sender: TObject);
    procedure PresetsComboDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
...
procedure TAdvancedAttackWaveFormatDesignerFrame.DrawTile(const AFrame: BInt32; const ARect: TRect);
var
  R: TRect;
begin
...
begin
  ...
  X := 0;
  Y := 0;
  while X < ComponentWidth do begin
    while Y < ComponentHeight do begin
      R := Rect(X, Y, X + TileDrawSize, Y + TileDrawSize);
      TileX := (X - PlayerDrawSize) div TileDrawSize;
      TileY := (Y div TileDrawSize) - AreaMaxExpansion;
      if (ClickedIncrement <> 0) and BInRange(TileX, 0, High(Expansion)) then
        if BInRange(ClickedX, R.Left, R.Right) and BInRange(ClickedY, R.Top, R.Bottom) then begin
          FExpansion[TileX] := BMinMax(FExpansion[TileX] + ClickedIncrement, 0, AreaMaxExpansion);
          ClickedIncrement := 0;
        end;
      if BInRange(TileX, 0, High(Expansion)) and (BAbs(TileY) <= Expansion[TileX] - 1) then
...
procedure TAdvancedAttackWaveFormatDesignerFrame.ExpansionFromText(const AText: BStr);
var
  I: BInt32;
begin
...
begin
  ...
  MainBuffer.SetBounds(0, PresetsCombo.Height, ComponentWidth, ComponentHeight);
  BackBuffer.SetBounds(0, PresetsCombo.Height, ComponentWidth, ComponentHeight);
...
procedure TAdvancedAttackWaveFormatDesignerFrame.MainBufferMouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TAdvancedAttackWaveFormatDesignerFrame.PresetsComboChange(Sender: TObject);
var
  S: BStr;
begin
  if PresetsCombo.ItemIndex <> -1 then begin
...
procedure TAdvancedAttackWaveFormatDesignerFrame.PresetsComboDrawItem(Control: TWinControl; Index: Integer; Rect: TRect;
  State: TOwnerDrawState);
begin
  BListDrawItem(PresetsCombo.Canvas, Index, odSelected in State, Rect, BStrLeft(PresetsCombo.Items[Index], ','));
end;

procedure TAdvancedAttackWaveFormatDesignerFrame.AutoRedrawTimer(Sender: TObject);
begin
  if ImVisible then
    Draw;
end;
