...
type
  TBBotDepositerStage = (bdsIdle, bdsFindDepot, bdsWalkToDepot, bdsCloseBackpacks, bdsOpenBackpacks, bdsOpenDepot,
    bdsDepositing, bdsAfterDepositing);
...
var
  ...
  Err: BUserError;
begin
  if DepositerStage <> bdsIdle then begin
    BBot.StandTime := Tick;
          ...
          for Y := -5 to 5 do
            if Tiles(Map, Me.Position.X + X, Me.Position.Y + Y) then
              if Map.IsDepot then begin
                ...
                if Score <> PathCost_NotPossible then begin
    ...
    bdsWalkToDepot: begin
        if Me.GetDistance(DepotPos) > 1 then begin
          if BBot.Walker.Task = nil then begin
            WalkPath := TBBotPathFinderPosition.Create('Depositer WalkTo <' + BStr(DepotPos) + '>');
            WalkPath.Position := DepotPos;
            WalkPath.Distance := 1;
            WalkPath.Execute;
            if WalkPath.Cost = PathCost_NotPossible then begin
    ...
    bdsCloseBackpacks: begin
        BBot.Walker.WaitLock('Depositer close backpacks', BBotDepositerDelayWalkLock);
    ...
    bdsOpenBackpacks: begin
        BBot.Walker.WaitLock('Depositer open backpacks', BBotDepositerDelayWalkLock);
        ActionNext.Lock(BBotDepositerDelayOpeningBackpacks);
        if (Tibia.TotalOpenContainers = 0) and (Me.Inventory.Backpack.IsContainer) then begin
          Me.Inventory.Backpack.UseAsContainer;
          Exit;
        end;
        if (Tibia.TotalOpenContainers = 1) and (Me.Inventory.Ammo.IsContainer) then begin
    ...
    bdsOpenDepot: begin
        BBot.Walker.WaitLock('Depositer open depot', BBotDepositerDelayWalkLock);
        ActionNext.Lock(BBotDepositerDelayOpeningBackpacks);
        CT := ContainerFirst;
        while CT <> nil do begin
          if CT.IsDepotContainer then begin
            Depot := CT.Container;
        ...
        if Tiles(Map, DepotPos) then begin
          if Map.Cleanup then
            Exit;
          if Map.IsDepot then begin
            Map.UseAsContainer;
    ...
    bdsDepositing: begin
        BBot.Walker.WaitLock('Depositer deposit items', BBotDepositerDelayWalkLock);
        ...
        if not CTDepot.Open then
          ...
          CT := ContainerFirst;
          while CT <> nil do begin
            if CT.LootDeposit and (CT.LootToContainer > 0) and (CT.Container <> Depot) then begin
              if CTDepot.PullHere(CT) = bcpsError then begin
                DepositerStage := bdsAfterDepositing;
                Err := BUserError.Create(Self, 'Unable to move items to the depot, possibly full depot');
          ...
          CT := ContainerFirst;
          while CT <> nil do begin
            if CT.Container <> Depot then begin
              if CT.IsContainer then
                Inc(Containers);
              if not Assigned(CT.Next) then
                Break;
              if (CT.Next.Container <> CT.Container) and (Containers <> 0) then
                Break;
            end;
            CT := CT.Next;
          end;
          ...
          if Assigned(CT) then
            if (Containers = 0) and (CT.Container <> Depot) then begin
              CT.Close;
          ...
          // End
          if Containers = 0 then begin
            DepositerStage := bdsAfterDepositing;
            Exit;
          end;
        end;
      end;
    bdsAfterDepositing: begin
        BBot.Walker.WaitLock('Depositer reset backpacks', BBotDepositerDelayWalkLock);
        DepositerStage := bdsIdle;
