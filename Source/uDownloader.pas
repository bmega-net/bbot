...
type
  TLoadURLState = (lurlSucess = 0, lurlErrorConnecting = 200,
    lurlErrorReadingHeaders = 201, lurlErrorReadingDataChuncked = 202,
    lurlErrorReadingData = 203, lurlErrorSendingHeaders);
...
procedure DownloadURL(AURL: BStr; AData: BStr; ACompletion: TLoadURLCompletion;
  AProgress: TLoadURLProgress);
...
type
  ...
  public
    constructor Create(AURL: BStr; AContents: BStr;
      ACompletion: TLoadURLCompletion; AProgress: TLoadURLProgress);
...
constructor TLoadURL.Create(AURL, AContents: BStr;
  ACompletion: TLoadURLCompletion; AProgress: TLoadURLProgress);
var
  Res: BStrArray;
  Request: BStr;
begin
  ...
  if not BSimpleRegex(URL_PATTERN, AURL, Res) then
  begin
    Host := '127.0.0.1';
    Exit;
  end;
  ...
  if Port = '' then
    Port := '80';
  SendData := '';
  SendData := SendData + IfThen(AContents = '', 'GET ', 'POST ') + Request +
    ' HTTP/1.1' + #13#10;
  SendData := SendData + 'Host: ' + Host + IfThen(Port <> '80',
    ':' + Port) + #13#10;
  SendData := SendData +
  ...
  begin
    SendData := SendData + 'Content-Length: ' + IntToStr(length(AContents)
      ) + #13#10;
    SendData := SendData +
...
begin
  ...
  repeat
    ...
    if AnsiPos('CONTENT-LENGTH', Buffer) = 1 then
      ProgressSize := StrToInt(Trim(Copy(Buffer, AnsiPos(':', Buffer) + 1,
        length(Buffer) - AnsiPos(':', Buffer) + 1)));
